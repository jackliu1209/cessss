...
async function fetchSongs() {
    try {
        const response = await fetch('https://api.qijieya.cn/meting/?type=playlist&id=12639909554');
...
// ✅ 修正: 保证 `currentSongIndex` 在合法范围内
function loadRandomSong() {
    if (songs.length === 0) return;

    currentSongIndex = Math.floor(Math.random() * songs.length);
    loadSelectedSong(songs[currentSongIndex]);
}
